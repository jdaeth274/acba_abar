...
def get_hit_csv(comM_hits_path, R_path, contig_dir):
    """ Function to run the R scripts for merging the conserved region hits
            directly after the BLAST search"""

    merge_cmd = "Rscript --vanilla " + R_path + "/extracting_comM_regions.R " + comM_hits_path + " " + contig_dir + " comM_gaps.csv"
...
def extract_comM(hit_csv, out_df):
    ...
    for index, row in hit_csv.iterrows():
        ...
        with open(current_fasta, "r") as fasta_seq:
            ...
            if current_row['ori'] == "forward":
                comM = fasta.seq[(current_row['hit_start'] - 1):current_row['hit_end']]
            else:
                comM = fasta.seq[(current_row['hit_end'] - 1):current_row['hit_start']].reverse_complement()

            comM_record = SeqRecord(comM, fasta.id + "_comM_gap_seq_" + str(current_row['hit_start']) + "-" + str(current_row['hit_end']), "","")
            comM_loc = out_df + "/" + str(current_row['id']) + "_comM_gap.fasta"
            SeqIO.write(comM_record, comM_loc, "fasta")
